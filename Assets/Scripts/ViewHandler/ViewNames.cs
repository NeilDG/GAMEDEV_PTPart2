using System.Collections;
using System.Collections.Generic;
using UnityEngine;

/// <summary>
/// Contains all view names used by the view handler.
/// Created BY: Neil DG
/// </summary>
public class ViewNames {
	public const string MAIN_MENU_PANEL_STRING = "MainMenuPanel";
	public const string SERVER_PANEL_STRING = "ServerPanel";
	public const string CLIENT_PANEL_STRING = "ClientPanel";
	public const string CHARACTER_SELECTION_PANEL_STRING = "CharacterSelectionPanel";
	public const string CHARACTER_PREVIEW_PANEL_STRING = "CharacterPreviewPanel";
	public const string LOBBY_PANEL_STRING = "LobbyPanel";
	public const string RESULTS_PANEL_STRING = "ResultsPanel";
	public const string QUIT_PANEL_STRING = "QuitScreen";
	public const string LOADING_SCREEN_STRING = "LoadingScreen";

	public const string TEST_PANEL_STRING = "TestPanel";
	public const string OBJECTIVE_PANEL_STRING = "ObjectiveScreen";

	public class DialogNames {
		public const string TWO_CHOICE_DIALOG_NAME = "TwoChoiceDialog";
		public const string NOTIFICATION_DIALOG_NAME = "NotificationDialog";
	}
}
